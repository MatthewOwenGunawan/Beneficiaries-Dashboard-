...
if not df.empty:
    ...
    def buat_pie(kolom, judul):
        if kolom in dff.columns and not dff[kolom].empty:
            counts = dff[kolom].value_counts().reset_index()
            counts.columns = [kolom, 'Jumlah']
            
            fig = px.pie(counts, names=kolom, values='Jumlah', hole=0)
            
            num_items = len(counts)
            x_domain = [0.15, 0.85] if num_items <= 5 else [0.1, 0.9]

            fig.update_traces(
                textposition='outside', 
                textinfo='percent+label', 
                marker=dict(colors=px.colors.qualitative.Pastel, line=dict(color='#0E1117', width=2)),
                domain=dict(x=x_domain, y=[0, 1])
            )

            fig.update_layout(
                title=dict(text=f"<b>{judul}</b>", x=0.5, y=0.95, xanchor='center', font=dict(color='white', size=16)),
    ...
    with r1_c1:
        ...
        fig1 = buat_pie('Sektor UMKM', "Sektor UMKM")
        if fig1: st.plotly_chart(fig1, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with r1_c2:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        fig2 = buat_pie('Rentang Usia', "Rentang Usia")
        if fig2: st.plotly_chart(fig2, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with r1_c3:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        fig3 = buat_pie('Kategori UMKM', "Skala Bisnis")
    ...
    with r2_c1:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        # PERUBAHAN: Sekarang menampilkan Kategori UMKM di baris kedua
        fig4 = buat_pie('Kategori UMKM', "Kategori UMKM")
        if fig4: st.plotly_chart(fig4, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with r2_c2:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        fig5 = buat_pie('Provinsi', "Sebaran Wilayah")
